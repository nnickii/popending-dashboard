...
$selectedPlant = filter_input(INPUT_GET, 'plant', FILTER_SANITIZE_STRING);
...
try {
    // Query Total Order Quantity
    $sqlOrder = "SELECT SUM(order_qty) AS total_order_qty FROM po_pending WHERE plant = :plant";
    $queryOrderqty = $conn->prepare($sqlOrder);
    $queryOrderqty->bindParam(':plant', $selectedPlant, PDO::PARAM_STR);
    $queryOrderqty->execute();
    $resultOrderqty = $queryOrderqty->fetch(PDO::FETCH_ASSOC);
    $OrderqtyCount = $resultOrderqty['total_order_qty'] ?? 0;

    // Query Factory Plant Orders
    $sqlFactory = "SELECT plant, COUNT(*) AS Factory FROM po_pending WHERE plant = :plant GROUP BY plant";
    $queryFactory = $conn->prepare($sqlFactory);
    $queryFactory->bindParam(':plant', $selectedPlant, PDO::PARAM_STR);
    $queryFactory->execute();
    $resultFactory = $queryFactory->fetch(PDO::FETCH_ASSOC);
    $FactoryCount = $resultFactory['Factory'] ?? 0;

    // Query Max PO Item
    $sqlpoitem = "SELECT po_item, COUNT(*) AS poitem_count FROM po_pending WHERE plant = :plant GROUP BY po_item 
                  ORDER BY poitem_count DESC LIMIT 1";
    $querypoitem = $conn->prepare($sqlpoitem);
    $querypoitem->bindParam(':plant', $selectedPlant, PDO::PARAM_STR);
    $querypoitem->execute();
    $resultpoitem = $querypoitem->fetch(PDO::FETCH_ASSOC);
    $poitemCount = $resultpoitem['po_item'] ?? 'N/A';
    $poitemCountOrders = $resultpoitem['poitem_count'] ?? 0;
    ...
    $sqlAging = "SELECT 
                ...
                WHERE plant = :plant
                GROUP BY aging_days";
    $queryAging = $conn->prepare($sqlAging);
    $queryAging->bindParam(':plant', $selectedPlant, PDO::PARAM_STR);
    $queryAging->execute();
    $agingData = $queryAging->fetchAll(PDO::FETCH_ASSOC);

    // Prepare aging data for table
    $agingRanges = ['0-15', '16-30', '31-60', '61-90', '91-120', '121-150', '151-180', '181-360', '>360'];
    $agingOrder = array_fill_keys($agingRanges, 0);
    $maxAging = 0;

    foreach ($agingData as $row) {
        $agingOrder[$row['aging_days']] = $row['orders'];
        $maxAging = max($maxAging, $row['orders']);
    }
    ...
    $query->bindParam(':plant', $selectedPlant, PDO::PARAM_STR);
    $query->execute();
    ...
    $result->bindParam(':plant', $selectedPlant, PDO::PARAM_STR);
    $result->execute();
    ...
    $sqlMax = "SELECT po_group, COUNT(*) AS Po_Pending FROM po_pending WHERE plant = :plant 
               GROUP BY po_group ORDER BY Po_Pending DESC";
    $queryMax = $conn->prepare($sqlMax);
    $queryMax->bindParam(':plant', $selectedPlant, PDO::PARAM_STR);
    $queryMax->execute();
    ...
    $sqlMin = "SELECT po_group, COUNT(*) AS Po_Pending FROM po_pending WHERE plant = :plant
               GROUP BY po_group ORDER BY Po_Pending ASC";
    $queryMin = $conn->prepare($sqlMin);
    $queryMin->bindParam(':plant', $selectedPlant, PDO::PARAM_STR);
    $queryMin->execute();
    ...
    $pogroupMax_json = json_encode($pogroupMax, JSON_UNESCAPED_UNICODE);
    $revenuesMax_json = json_encode($revenuesMax, JSON_UNESCAPED_UNICODE);
    $pogroupMin_json = json_encode($pogroupMin, JSON_UNESCAPED_UNICODE);
    $revenuesMin_json = json_encode($revenuesMin, JSON_UNESCAPED_UNICODE);
                ...
                <li class="nav-item dropdown">
                    <form method="get" action="dashboard.php" class="d-flex">
                        <select name="plant" class="form-select" onchange="this.form.submit();"
                            ...
                            <?php
                            $sqlPlant = "SELECT DISTINCT plant FROM po_pending ORDER BY plant";
                            foreach ($conn->query($sqlPlant) as $row) {
                                $plant = htmlspecialchars($row['plant']);
                                $selected = $plant === $selectedPlant ? 'selected' : '';
                    ...
                    <div class="row">
                        <div class="col-xl-4 col-md-6 mb-4">
                            <div class="card bg-primary text-white shadow">
                                ...
                                <p class="card-text ps-3"><?php echo htmlspecialchars($OrderqtyCount); ?> Quantity</p>
                            </div>
                        </div>
                        <div class="col-xl-4 col-md-6 mb-4">
                            <div class="card bg-warning text-white shadow">
                                <div class="card-body">Factory Order</div>
                                <p class="card-text ps-3"><?php echo htmlspecialchars($FactoryCount); ?> Orders</p>
                            </div>
                        </div>
                        <div class="col-xl-4 col-md-6 mb-4">
                            <div class="card bg-success text-white shadow">
                                <div class="card-body">Max PO Item</div>
                                <p class="card-text ps-3"><?php echo htmlspecialchars($poitemCount); ?> (<?php echo htmlspecialchars($poitemCountOrders); ?> Orders)</p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    ...
                                    foreach ($agingRanges as $range) {
                                        echo "<th><span>{$range}</span><br><span>Days</span></th>";
                                    }
                                    echo "</tr></thead><tbody>";
                                    echo "<tr><td>Count</td>";
                                    foreach ($agingRanges as $range) {
                                        ...
                                        if (empty($selectedPlant)) {
                                            echo "<td colspan='2' style='color: red;'>Plant is required</td>";
                                        } else {
                                            echo "<td $highlight>";
                                            echo "<a href='aging.php?aging=" . urlencode($selectedPlant) . "&aging=" . urlencode($range) . "&plant=" . urlencode($selectedPlant) .
                                                "' style='text-decoration: none; color: inherit;'>{$order}</a>";
                                            echo "</td>";
                                        }
                                    }
                                    echo "</tr>";
                        ...
                        <div class="col-xl-6 mb-4">
                            <div class="card">
                                ...
                                <div class="card-body" style="width: 100%; margin: 5px;">
                                    ...
                                    <script>
                                        // ข้อมูลและการตั้งค่าสำหรับ Pie Chart
                                            ...
                                            options: {
                                                ...
                                                onClick: function(event, elements) {
                                                    if (elements.length > 0) {
                                                        ...
                                                        window.location.href = `status.php?status=${encodeURIComponent(status)}&plant=${encodeURIComponent(plant)}`;
                                                    }
                                                }
                                            },
                                            ...
                                            new Chart(ctxHor, {
                                                ...
                                                options: {
                                                    ...
                                                    onClick: function(event, elements) {
                                                        if (elements.length > 0) {
                                                            const index = elements[0].index;
                                                            const mat_desc = chartData.labels[index];
                                                            const plant = "<?php echo htmlspecialchars($selectedPlant); ?>";
                                                            window.location.href = `material_details.php?mat_desc=${encodeURIComponent(mat_desc)}&plant=${encodeURIComponent(plant)}`;
                                                        }
                                                    }
                                                }
                                            });
